...
def test_processed_KEGG_rule0023_and_rule0024_rxns_count(KEGG_df):
    ...
    assert rule0023_df['substrates'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the products column for carboxylation rxns (rule0023) should contain a list with exactly one element each
    assert rule0023_df['products'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the substrates column for decarboxylation rxns (rule0024) should contain a list with exactly one element each
    assert rule0024_df['substrates'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the products column for decarboxylation rxns (rule0024) should contain a list with exactly one element each
    assert rule0024_df['products'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the lhs cofactors column for carboxylation rxns (rule0023) should contain exactly one element each
    assert rule0023_df['LHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the rhs cofactors column for carboxylation rxns (rule0023) should contain no cofactors
    assert rule0023_df['RHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 0)).all()

    # the lhs cofactors column for decarboxylation rxns (rule0024) should contain no cofactors
    assert rule0024_df['LHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 0)).all()

    # the rhs cofactors column for decarboxylation rxns (rule0024) should contain exactly one element each
    assert rule0024_df['RHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the lhs cofactor codes column for carboxylation rxns (rule0023) should contain exactly one element: CO2
    assert rule0023_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1 and x[0] == 'CO2')).all()

    # the rhs cofactor codes column for carboxylation rxns (rule0023) should contain no cofactors
    assert rule0023_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 0)).all()

    # the lhs cofactor codes column for decarboxylation rxns (rule0024) should contain no cofactors
    assert rule0024_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 0)).all()

    # the rhs cofactor codes column for decarboxylation rxns (rule0024) should contain exactly one element: CO2
    assert rule0024_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1 and x[0] == 'CO2')).all()
...
def test_processed_BRENDA_rule0023_and_rule0024_rxns_count(BRENDA_df):
    ...
    # the rhs cofactors column for carboxylation rxns (rule0023) should contain no cofactors
    assert rule0023_df['RHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the lhs cofactors column for decarboxylation rxns (rule0024) should contain no cofactors
    assert rule0024_df['LHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()
...
def test_processed_KEGG_aldehyde_dehydrogenase_rules(KEGG_df):
    ...
    assert rule0025_df['substrates'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the products column for rule0025 should contain exactly one element: the carboxylic acid produced
    assert rule0025_df['products'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the substrates column for rule0026 should contain exactly one element: the carboxylic acid being oxidized
    assert rule0026_df['substrates'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the products column for rule0026 should contain exactly one element: the aldehyde produced
    assert rule0026_df['products'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the lhs cofactors column for rule0025 should contain exactly one element
    assert rule0025_df['LHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the rhs cofactors column for rule0025 should contain exactly two elements
    assert rule0025_df['RHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 2)).all()

    # the lhs cofactors column for rule0026 should contain exactly two elements
    assert rule0026_df['LHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 2)).all()

    # the rhs cofactors column for rule0026 should contain exactly one element
    assert rule0026_df['RHS_cofactors'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1)).all()

    # the lhs cofactor codes column for rule0025 should contain exactly one element: NADH_CoF
    assert rule0025_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1 and x[0] == 'NADH_CoF')).all()

    # the rhs cofactor codes column for rule0025 should contain exactly two elements: NAD_CoF and WATER
    assert rule0025_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 2 and 'NAD_CoF' in x and 'WATER' in x)).all()

    # the lhs cofactor codes column for rule0026 should contain exactly two elements: NAD_CoF and WATER
    assert rule0026_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 2 and 'NAD_CoF' in x and 'WATER' in x)).all()

    # the rhs cofactor codes column for rule0026 should contain exactly one element: NADH_CoF
    assert rule0026_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, np.ndarray) or (isinstance(x, list) and len(x) == 1 and x[0] == 'NADH_CoF')).all()
...
def test_processed_MetaCyc_aldehyde_dehydrogenase_rules(MetaCyc_df):
    ...
    # the substrates column for rule0025 should contain exactly one element: the aldehyde being oxidized
    assert rule0025_df['substrates'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the products column for rule0025 should contain exactly one element: the carboxylic acid produced
    assert rule0025_df['products'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the substrates column for rule0026 should contain exactly one element: the carboxylic acid being oxidized
    assert rule0026_df['substrates'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the products column for rule0026 should contain exactly one element: the aldehyde produced
    assert rule0026_df['products'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the lhs cofactors column for rule0025 should contain exactly one element
    assert rule0025_df['LHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the rhs cofactors column for rule0025 should contain exactly two elements
    assert rule0025_df['RHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 2).all()

    # the lhs cofactors column for rule0026 should contain exactly two elements
    assert rule0026_df['LHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 2).all()

    # the rhs cofactors column for rule0026 should contain exactly one element
    assert rule0026_df['RHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the lhs cofactor codes column for rule0025 should contain exactly one element: NADH_CoF
    assert rule0025_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 1 and x[0] == 'NADH_CoF').all()

    # the rhs cofactor codes column for rule0025 should contain exactly two elements: NAD_CoF and WATER
    assert rule0025_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 2 and 'NAD_CoF' in x and 'WATER' in x).all()

    # the lhs cofactor codes column for rule0026 should contain exactly two elements: NAD_CoF and WATER
    assert rule0026_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 2 and 'NAD_CoF' in x and 'WATER' in x).all()

    # the rhs cofactor codes column for rule0026 should contain exactly one element: NADH_CoF
    assert rule0026_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 1 and x[0] == 'NADH_CoF').all()
...
def test_processed_KEGG_monooxygenase_rules(KEGG_df):
    ...
    assert rule0004_df['substrates'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the products column for rule0004 should contain exactly one element: the monooxygenated product
    assert rule0004_df['products'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the substrates column for rule0005 should contain exactly one element: the monooxygenated product
    assert rule0005_df['substrates'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the products column for rule0005 should contain exactly one element: the substrate being monooxygenated
    assert rule0005_df['products'].apply(lambda x: isinstance(x, list) and len(x) == 1).all()

    # the lhs cofactors column for rule0004 should contain exactly two elements
    assert rule0004_df['LHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 2).all()

    # the rhs cofactors column for rule0004 should contain exactly two elements
    assert rule0004_df['RHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 2).all()

    # the lhs cofactors column for rule0005 should contain exactly two elements
    assert rule0005_df['LHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 2).all()

    # the rhs cofactors column for rule0005 should contain exactly two elements
    assert rule0005_df['RHS_cofactors'].apply(lambda x: isinstance(x, list) and len(x) == 2).all()

    # the lhs cofactor codes column for rule0004 should contain exactly two elements: NADH_CoF and OXYGEN
    assert rule0004_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 2 and 'NADH_CoF' in x and 'O2' in x).all()

    # the rhs cofactor codes column for rule0004 should contain exactly two elements: NAD_CoF and WATER
    assert rule0004_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 2 and 'NAD_CoF' in x and 'WATER' in x).all()

    # the lhs cofactor codes column for rule0005 should contain exactly two elements: NAD_CoF and WATER
    assert rule0005_df['LHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 2 and 'NAD_CoF' in x and 'WATER' in x).all()

    # the rhs cofactor codes column for rule0005 should contain exactly two elements: NADH_CoF and OXYGEN
    assert rule0005_df['RHS_cofactor_codes'].apply(lambda x: isinstance(x, list) and len(x) == 2 and 'NADH_CoF' in x and 'O2' in x).all()
